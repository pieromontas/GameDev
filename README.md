...
- **E-priority** — chest → spring → grove herb → shrine → gate guard → blacksmith → plaza baker/tailor/apothecary → street vendor → produce stall → traveling cart → market sign → notice board → inn → alley → harbor catch crate → residential door → town chapel → merchant (merchant never blocks closer interactables)
...
src/
  ...
  world/MarketDistrict.ts    NE market sign + blacksmith + plaza shop doors + produce stall + traveling cart + notice board + inn + alley interacts (E)
