...
On a phone browser (Chrome / Safari), coarse-pointer detection shows a left analog stick, right-half look drag (not an attack), and a lower-right **1–4 / Dodge / E** cluster. Desktop WASD, LMB attack, and RMB look stay unchanged. Append `?touch=1` to the URL to preview the overlay on a desktop mouse.
...
src/
  ...
  world/MarketDistrict.ts    NE market sign + blacksmith + produce stall + notice board + inn + alley interacts (E)
