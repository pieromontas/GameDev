...
- **E-priority** — chest → spring → grove herb → shrine → gate guard → blacksmith → street vendor → produce stall → traveling cart → market sign → notice board → inn → alley → harbor catch crate → residential door → town chapel → merchant (merchant never blocks closer interactables)
...
src/
  ...
  world/MarketDistrict.ts    NE market sign + blacksmith + produce stall + traveling cart + notice board + inn + alley interacts (E)
